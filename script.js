...
function createChoroplethMap() {
   loadData(() => {
     ...
     const svg = d3.select("#mapChart")
       .append("svg")
       .attr("width", width)
       .attr("height", height + 60); // Extra height for padding at the top
     ...
     svg.append("text")
       ...
       .style("fill", "#4a90e2")
       .style("z-index", "10") // Ensure title is on top
       .text("Gini Index by Country (Income Inequality)");
 
     svg.append("text")
       .attr("x", width / 2)
       .attr("y", 50) // Increase space below the title
       .attr("text-anchor", "middle")
       .style("font-size", "12px")
       .style("fill", "gray")
       .style("z-index", "10") // Ensure subtitle is on top
     ...
     d3.json("https://raw.githubusercontent.com/holtzy/D3-graph-gallery/master/DATA/world.geojson").then(world => {
       svg.selectAll("path")
         ...
         .style("z-index", "1") // Make sure map is below title and legend
     ...
     // Improved Legend with adjusted position and z-index
     const legendWidth = 200, legendHeight = 10;
     const legend = svg.append("g")
       .attr("transform", `translate(${width - 220}, 80)`)
       .style("z-index", "10"); // Bring legend to the front
